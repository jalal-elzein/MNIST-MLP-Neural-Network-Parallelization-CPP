...
void perceptron() {
    memset(in2, 0, sizeof(float) * n2);
    memset(in3, 0, sizeof(float) * n3);
...
int main(int argc, char *argv[]) {
    ...
    for (int sample = 1; sample <= nTesting; ++sample) {
        ...
        if (label == predict) {
            ++nCorrect;
            // cout << "Classification: YES. Label = " << label << ". Predict =
            // " << predict << endl << endl;
            report << "Sample " << sample << ": YES. Label = " << label
                   << ". Predict = " << predict << ". Error = " << error
                   << endl;
        } else {
            // cout << "Classification: NO.  Label = " << label << ". Predict =
            // " << predict << endl; cout << "Image:" << endl; for (int j = 1; j
            // <= height;
            // ++j) { 	for (int i = 1; i <= width; ++i) { 		cout <<
            // d[i][j];
            // 	}
            // 	cout << endl;
            // }
