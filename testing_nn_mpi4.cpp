...
int main(int argc, char *argv[]) {
    ...
    // Divide the testing dataset into equal-sized chunks

    int chunk_size = nTesting / numprocs;

    int *label_data = nullptr;

    double *image_data = nullptr;
...

    {
        ...
        std::ifstream label_file("t10k-labels.idx1-ubyte", std::ios::binary);

        std::ifstream image_file("t10k-images.idx3-ubyte", std::ios::binary);

        label_data = new int[nTesting];

        image_data = new double[nTesting * n1];

        // Read label data

        // label_file.seekg(8); // Skip header
...

        {
            ...
            label_data[i] = static_cast<int>(label);
        }

        // Read image data

        // image_file.seekg(16); // Skip header

        for (int i = 0; i < nTesting * n1; i++)

        {
            ...
            image_data[i] =
                static_cast<double>(static_cast<unsigned char>(pixel)) / 255.0;
        }
    }

    // Allocate memory for the chunk of images and labels

    int num_samples = chunk_size * (rank == 0 ? (nTesting % numprocs) : 1);

    int *labels = new int[num_samples];

    double *images = new double[num_samples * n1];

    // Scatter the images and labels to all processes

    MPI_Bcast(&chunk_size, 1, MPI_INT, 0, MPI_COMM_WORLD);

    MPI_Scatter(label_data, chunk_size, MPI_INT, labels, num_samples, MPI_INT,
                0, MPI_COMM_WORLD);

    MPI_Scatter(image_data, chunk_size * n1, MPI_DOUBLE, images,
                num_samples * n1, MPI_DOUBLE, 0, MPI_COMM_WORLD);
    ...
    // Reduce the testing results to the root process and compute the total
    // number of correctly classified images

    int *results = new int[numprocs];

    // Allocate memory for total_correct on the root process
    ...
    MPI_Reduce(&nCorrect, &total_correct, 1, MPI_INT, MPI_SUM, 0,
               MPI_COMM_WORLD);

    MPI_Finalize();

    // Deallocate the memory allocated for results
